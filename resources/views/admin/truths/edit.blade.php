...
@section('title', 'Edit Truth')

@section('content')
<div class="mb-8">
    <h1 class="text-4xl font-bold mb-2">Edit Truth Question</h1>
    <p class="text-gray-600">Update the truth question</p>
</div>

<div class="bg-white border-4 border-black p-8 max-w-2xl">
    <form action="{{ route('admin.truths.update', $truth) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea 
                ...
                required
            >{{ old('question', $truth->question) }}</textarea>
            @error('question')
        ...
        <div class="mb-6">
            <label class="flex items-center">
                <input type="checkbox" name="is_active" class="w-4 h-4 border-2 border-black" {{ $truth->is_active ? 'checked' : '' }}>
